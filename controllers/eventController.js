...
exports.upload = upload.fields([
    { name: 'eventImage', maxCount: 1 },   // Field name should match the form input name
    { name: 'speakerImage', maxCount: 4 }  // You can add more fields if needed
]);
...
// exports.addEvent = async (req, res) => {
//     const { titre, description, date_debut, date_fin, lieu } = req.body;
//     const eventImage = req.files['eventImage'] ? '/images/' + req.files['eventImage'][0].filename : null;
//     const sponsorsData = JSON.parse(req.body.sponsorsData || '[]');
//     const speakersData = JSON.parse(req.body.speakersData || '[]');
//     const programmeDaysData = JSON.parse(req.body.programDaysData || '[]');
...
exports.addEvent = async (req, res) => {
    ...
    try {
        ...
        for (let i = 0; i < speakersData.length; i++) {
            ...
            const speakerImage = req.files['speakerImage'] && req.files['speakerImage'][i] ? '/images/' + req.files['speakerImage'][i].filename : null;
            await db.query('INSERT INTO Speakers (evenement_id, nom, prenom, image_url, description) VALUES (?, ?, ?, ?, ?)', [eventId, speaker.name, speaker.surname, speakerImage, speaker.description]);
        }
...
exports.getEventDetails = async (req, res) => {
    ...
    try {
        ...
        // Récupérer les informations sur l'utilisateur s'il est connecté
